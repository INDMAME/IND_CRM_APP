...
import { normalizeVisibleVisitUsers, type DataVisibilityVisibleUser } from "../utils/visibleVisitUsers.ts";
...
const CACHE_PREFIX = "visits_visible_users_v2";
...
const hasPreloadedVisibleUsers = (): boolean => {
  return typeof window !== "undefined" && Array.isArray(window.__IND_VISIBLE_VISIT_USERS__);
};

const readPreloadedVisibleUsers = (): DataVisibilityVisibleUser[] => {
...
export const useVisibleVisitUsers = ({
  ...
  const [visibleVisitUsers, setVisibleVisitUsers] = useState<DataVisibilityVisibleUser[]>(() => readPreloadedVisibleUsers());
  const [visibleUsersLoading, setVisibleUsersLoading] = useState(false);
  const [visibleUsersError, setVisibleUsersError] = useState("");
  const [visibleUsersReady, setVisibleUsersReady] = useState(() => hasPreloadedVisibleUsers());
