...
const ChatMarkdownMessage = ({ markdown, className }: ChatMarkdownMessageProps) => {
  return (
    <div className={classNames("min-w-0 break-words text-sm leading-6", className)}>
      <ReactMarkdown
        skipHtml
        components={{
          ...
          h2: ({ children }) => <h2 className="mb-2 text-sm font-semibold leading-6">{children}</h2>,
          h3: ({ children }) => <h3 className="mb-1 text-sm font-semibold leading-6">{children}</h3>,
          a: ({ href, children }) => (
            <a
              href={href}
              target="_blank"
          ...
          pre: ({ children }) => (
            <pre className="overflow-x-auto rounded-xl bg-slate-900 px-3 py-2 text-xs text-slate-50">{children}</pre>
          ),
          blockquote: ({ children }) => (
