...
export type UseTimelineCardEffectsArgs = {
  containerRef: React.RefObject<HTMLDivElement | null>;
  errorMessage: string;
  items: unknown[];
  resolveClickableCard: (target: EventTarget | null) => HTMLElement | null;
};
...
const applyEllipsis = (el: HTMLElement, fullText: string, multiLine: boolean) => {
  ...
  if (multiLine) {
    ...
    // Some browsers return unitless line-height values for computed styles.
    // Convert tiny unitless values using font-size to avoid collapsing text.
...
// Owns tooltip, ellipsis, and pixel effects for timeline cards.
export const useTimelineCardEffects = ({
  containerRef,
  errorMessage,
  items,
  resolveClickableCard,
}: UseTimelineCardEffectsArgs) => {
